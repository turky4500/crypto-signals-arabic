...
| [`alerts.py`](alerts.py) | بوابة التنبيه عالي القناعة (12 شرطاً) + صياغة رسائل واتساب + الإرسال |
| [`backtest.py`](backtest.py) | اختبار تاريخي على بيانات فعلية مع رسوم وانزلاق، ومقارنة بين الإعدادات |
| [`scripts/gate_study.py`](scripts/gate_study.py) | أداة قياس بوابة التنبيه — 120 توليفة على 286 ألف شمعة |
...
| [`tests/test_alerts.py`](tests/test_alerts.py) | 55 اختباراً للبوابة والأسعار والرسائل والإرسال |
| [`tests/test_theme.py`](tests/test_theme.py) | 39 اختباراً للسمة وتكافؤ الرموز و26 نسبة تباين WCAG |
| [`docs/alert-gate-study.md`](docs/alert-gate-study.md) | دراسة قياس بوابة التنبيه — النتيجة السالبة معلنة |
| [`backtest-report.md`](backtest-report.md) | نتيجة الاختبار التاريخي المُلْتَزَمة |
| [`SECURITY.md`](SECURITY.md) | سياسة الأمان + جدول الأسرار وقواعد عدم التسريب |
...
## اللوحة: الوضعان والتحديث المستمر

### الوضع النهاري والليلي

زر في شريط الحالة يبدّل بين الوضعين، واختيارك **يُحفظ في المتصفح** ويعود معك.

- **الافتراضي يتبع نظامك** عبر `prefers-color-scheme`، وإن غيّرت تفضيل النظام أثناء
  التصفح تتبعه اللوحة فوراً — ما لم تكن اخترت وضعاً بنفسك، فاختيارك أولى.
- **بلا وميض عند الفتح**: سكربت من ثلاثة أسطر يضبط السمة في `<head>` قبل أول رسم.
- **بلا كسر في التصفح الخاص**: غياب `localStorage` أو `matchMedia` لا يُسقط الإقلاع.
- `meta[theme-color]` يتحدّث مع الوضع فيتطابق شريط المتصفح في الهاتف.
- الانتقال بين الوضعين حركة ناعمة تُعطَّل تلقائياً مع `prefers-reduced-motion`.

**كل لون في الصفحة رمز CSS** — لا يوجد لون واحد مكتوب يدوياً خارج `:root`، فالتبديل
يحدث دفعة واحدة ولا يبقى عنصر عالقاً في الوضع الخطأ. و`tests/test_theme.py` يفرض
ذلك ويمنع عودة أي لون مكتوب يدوياً.

#### التباين مقاس لا مُقدَّر

فُحصت 13 تركيبة نص/خلفية × وضعين = **26 زوجاً**، وكلها تتجاوز **4.5:1** (WCAG 2.1 AA):

| | الليلي | النهاري |
|---|---|---|
| الأدنى بين كل الأزواج | **4.58:1** | **4.52:1** |
| النص الأساسي | 16.52:1 | 17.37:1 |

الفحص كشف خللاً كان موجوداً **قبل** إضافة الوضع النهاري: `--dim` (تسميات 11px ورأس
الجدول) كان 3.46:1 في الوضع الليلي — تحت الحد. صُحّح إلى 4.58:1. ألوان الوضع
النهاري مُعتَّمة عمداً عن ألوان الليلي، لأن عكسها كما هو يفشل في التباين.

### التحديث المستمر

| | قبل | بعد |
|---|---|---|
| تكرار الفحص في الصفحة | كل 5 دقائق | **كل 60 ثانية** |
| تكرار سير العمل | كل ساعة عند :05 | **كل 20 دقيقة** (5، 25، 45) |
| عند العودة إلى التبويب | ينتظر الدورة | **يفحص فوراً** |
| عند انقطاع الشبكة وعودتها | لا شيء | **يفحص فوراً** |
| عند الفشل المتكرر | يحاول كل 5 دقائق أبداً | **تراجع أُسّي** 60→120→…→600 ثانية |
| والنافذة مخفية | يستمر بالاستهلاك | **يتوقف** ويوفر الشبكة والبطارية |

#### لماذا كانت رسالة «البيانات قديمة» تظهر بلا سبب

علّتان مستقلتان اجتمعتا:

1. **قياس خاطئ في الصفحة.** كانت الطزاجة تُحسب من `as_of` — وهو **وقت إغلاق الشمعة**
   لا وقت الفحص. فالبيانات تبدو «عمرها ساعة» في نهاية كل ساعة حتى لو فحص الماسح
   قبل دقيقتين. صارت تُحسب من عدد **الشموع المغلقة الفائتة** ومن `generated_at`
   (وقت الفحص الفعلي)، وكلاهما معروض.

2. **جدولة GitHub Actions متأخرة بطبعها.** رُصد تأخير **42 دقيقة** على هذا المستودع
   عن موعد `:05`. بإضافة فتحتَي `:25` و`:45` صار أقصى تأخير عملي نحو 20 دقيقة،
   والفتحتان تعملان شبكة أمان إن فشل تشغيل أو تأخر.

#### الحالات الأربع الآن

| الحالة | متى | الرسالة |
|---|---|---|
| 🟢 محدَّثة | الشمعة المحلَّلة هي أحدث شمعة مغلقة | «النظام يعمل — بيانات محدَّثة» |
| 🟢 بانتظار | شمعة جديدة أُغلقت ولسنا بعدُ في مهلة النشر (7 د) | «بانتظار شمعة الساعة الجديدة» |
| 🟠 متأخرة | تجاوزنا المهلة ولم تصل شمعة الساعة | «الدورة متأخرة N دقيقة — يُعاد المحاولة تلقائياً» |
| 🔴 متوقفة | **شمعتان مغلفتان فأكثر** فاتتنا | «البيانات قديمة — فاتتنا N شموع ساعة…» |

> **لماذا «شمعتان» هي عتبة التوقّف؟** الاعتماد على التأخير وحده جعل حالة «متوقف»
> **مستحيلة رياضياً**: أقصى تأخير ممكن داخل الساعة الواحدة نحو 48 دقيقة، وهي دون
> أي عتبة معقولة دائماً. كشفت هذا الاختبارات المُجمِّدة للساعة لا المراجعة البصرية.

وأُضيفت قراءة **«آخر تحقق: قبل N ثانية»** في شريط الحالة — تميّز بين «الصفحة
جامدة» و«لا جديد بعد»، وهو ما كان غائباً.

العدّاد التنازلي يشير الآن إلى **فتحة التشغيل الفعلية القادمة** (5/25/45) لا إلى
`as_of + ساعة`، فاختفت رسالة «الدورة الجديدة مستحقة — يُتحقق تلقائياً» المعلَّقة.

ولا تُعاد رسم الجدول إلا إذا تغيّرت البيانات فعلاً (مقارنة `generated_at` و`as_of`)،
فيبقى تركيز لوحة المفاتيح وموضع التمرير كما هما ولا يُستهلك جهد بلا فائدة كل دقيقة.
...
## تنبيهات واتساب — الفرص عالية القناعة

وحدة [`alerts.py`](alerts.py) ترسل رسالة واتساب عند ظهور فرصة تتجاوز **بوابة من 12 شرطاً**
مصمّمة لـ**السبوت، شراءً فقط** (بلا رافعة وبلا بيع على المكشوف).

### معايير «الفرصة المؤكدة» — الاثنا عشر شرطاً

كلها يجب أن تتحقق معاً، وأي شرط واحد ناقص يمنع الإرسال:

| # | الشرط | القيمة | لماذا |
|---|---|---|---|
| 1 | تتضمن **استراتيجية A** (استمرار الاتجاه) | إلزامي | الوحيدة التي كانت موجبة داخل العينة |
| 2 | **تأكيدان** فأكثر معاً | ≥ 2 | تقاطع الاستراتيجيات يقلّل الإشارات الزائفة |
| 3 | **النتيجة** المركّبة | ≥ 62 | خطّ فرز أعلى من عتبة اللوحة |
| 4 | **ترتيب صاعد سليم** | السعر > EMA20 > EMA50 > EMA200 | لا شراء ضد الاتجاه العام |
| 5 | **بلا تحذيرات خروج** | X1–X4 = 0 | لا دخول وشيء يشير إلى الخروج |
| 6 | **السيولة النسبية** | ≥ 1.8x من متوسط 20 شمعة | زخم حقيقي لا ضجيج |
| 7 | **حجم 24 ساعة** | ≥ 10M USDT | انزلاق سعري منخفض |
| 8 | **تغيّر 24 ساعة** | بين 2% و7% | فوق 7% تكون قد فاتتك الحركة |
| 9 | **RSI** | بين 53 و65 | زخم بلا تشبّع شرائي |
| 10 | **التذبذب (ATR)** | بين 1.0% و3.2% | أقل من 1% لا يبلغ الهدف؛ فوق 3.2% يمزّق الوقف |
| 11 | **البعد عن EMA20** | ≤ 2.5% | لا شراء من وضع ممتد |
| 12 | **البعد عن قمة 24 ساعة** | ≥ 1.5% | لا شراء من القمة تماماً |

### محتوى الرسالة

```text
💰 سعر إصدار الإشارة   ← إغلاق الشمعة التي أطلقت الإشارة
📥 سعر الدخول          ← السعر اللحظي وقت الإرسال (يُجلب من Binance مباشرة)
🎯 سعر الخروج (الهدف)  ← الدخول + 3%
🛑 وقف الخسارة         ← الدخول − 2%
📊 المخاطرة/العائد     ← 1 : 1.5
```

بالإضافة إلى أسباب تجاوز البوابة، ومؤشرات الشمعة، **وفقرة الاحتمالات الحقيقية**.

### ⚠️ الأرقام الصادقة — اقرأها قبل التفعيل

البوابة لم تُختر ذوقاً بل **قِيست**. راجع
[`docs/alert-gate-study.md`](docs/alert-gate-study.md): 120 توليفة
(6 مستويات تشدّد × 4 أهداف × 5 وقفات) على **286,347 شمعة × 40 زوجاً × 300 يوماً**،
برسوم Binance الحقيقية (0.1% لكل طرف) وانزلاق 0.05%.

| المقياس | الهدف 3% (الافتراضي) | الهدف 1% |
|---|---|---|
| إصابة الهدف تاريخياً | **41.1%** | 47.4% |
| نقطة التعادل المطلوبة | 44.7% | 62.5% |
| صافي العائد لكل صفقة | **−0.18%** | −0.30% |
| عامل الربح | 0.865 | 0.44 |

**لم تحقق أي توليفة من الـ120 توقعاً إيجابياً.** الأعلى كان 0.865.

ولهذا:

- **الهدف الافتراضي 3% لا 1%** — عند 1% تبتلع الرسوم 20% من الهدف الإجمالي، فتصبح
  نقطة التعادل 62.5% بينما أعلى ما قِيس 47.4%. **الهدف 1% غير قابل للتحقيق بنيوياً** بهذه الإشارات.
- **كل رسالة تحمل احتمالاتها الحقيقية** داخلها. من حق المتلقي أن يعرف أن نسبة
  الإصابة 41% وأن التوقع سالب، بدل أن توحي الرسالة بيقين غير موجود.
- يمكنك مع ذلك تجربة هدف 1% بـ `--alert-tp-pct 1.0`، وستُستبدل الفقرة تلقائياً
  بأرقام 1% الخاصة.

هذه **أداة فرز ومراقبة** — تختصر 163 زوجاً إلى فرصة أو فرصتين في اليوم تستحق النظر.
ليست توصية شراء وليست نصيحة مالية.

### التفعيل

أضف ثلاثة أسرار في **Settings → Secrets and variables → Actions**:

| السر | القيمة |
|---|---|
| `WHATSAPP_API_URL` | رابط واجهة الإرسال (`.../api/v1/send`) |
| `WHATSAPP_API_TOKEN` | توكن Bearer |
| `WHATSAPP_TO` | الرقم بصيغة دولية دون `+` (مثال: `9665XXXXXXXX`) |

**لا سر ⇒ لا تنبيهات**، والماسح يعمل كالمعتاد. ولا تُطبع القيم في سجل التشغيل.

محلياً:

```bash
export WHATSAPP_API_URL='https://…/api/v1/send'
export WHATSAPP_API_TOKEN='…'
export WHATSAPP_TO='9665XXXXXXXX'
python scanner.py --whatsapp-dry-run   # اصنع الرسائل واطبعها دون إرسال
```
...
|---|---|---|
| `--alert-tp-pct` | 3.0 | نسبة الهدف من سعر الدخول |
| `--alert-sl-pct` | 2.0 | نسبة وقف الخسارة |
| `--alert-max` | 3 | أقصى تنبيهات في الدورة الواحدة |
| `--alert-cooldown` | 24 | ساعات قبل إعادة تنبيه نفس الزوج |
| `--alert-min-score` | 62 | أدنى نتيجة مركّبة |
| `--alert-min-confirmations` | 2 | أدنى تأكيدات |
| `--alert-min-vol-ratio` | 1.8 | أدنى سيولة نسبية |
| `--whatsapp-dry-run` | — | اصنع الرسائل دون إرسال |
| `--alert-no-stats` | — | احذف فقرة الاحتمالات (**غير مستحسن**) |

**التكرار المتوقع:** نحو 1.3 تنبيه في اليوم على الكون الإنتاجي (163 زوجاً) —
قِيست البوابة الأشد عند 100 إشارة على 40 زوجاً × 300 يوماً.

### كيف تبقى فترة التبريد بلا تخزين حالة؟

سجل التنبيهات (`alert_log`) يُكتب داخل `signals.json` المنشور نفسه، وتقرؤه الدورة
التالية عبر `SIGNALS_STATE_URL`. فلا تُلتزم أي حالة في المستودع، ويبقى تاريخ
`main` نظيفاً — نفس آلية `first_seen` تماماً.
