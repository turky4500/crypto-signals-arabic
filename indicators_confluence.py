...
from indicators import ema, macd, rsi, supertrend
...
def analyze_confluence(df, timeframe="4h"):
    ...
    _macd_v, _macd_s, macd_h = macd(df["c"])
    ...
    if ema9 > ema21:
        indicators["ema_cross"] = {
            "vote": "BULLISH",
            "strength": 80,
            "detail": f"EMA9 ({ema9:.4f}) > EMA21 ({ema21:.4f})",
        }
        score += 15
    elif ema9 < ema21:
        indicators["ema_cross"] = {
    ...
    if close > ema50 > ema200:
        indicators["ema_trend"] = {"vote": "BULLISH", "strength": 90, "detail": "Price > EMA50 > EMA200"}
        score += 15
    elif close < ema50 < ema200:
        indicators["ema_trend"] = {"vote": "BEARISH", "strength": 90, "detail": "Price < EMA50 < EMA200"}
        score += 0
    else:
    ...
    if st_direction == 1:
        indicators["supertrend"] = {
            "vote": "BULLISH",
            "strength": 85,
            "detail": f"SuperTrend UP at {st_line.iloc[-1]:.4f}",
        }
        score += 20
    elif st_direction == -1:
        indicators["supertrend"] = {
            "vote": "BEARISH",
            "strength": 85,
            "detail": f"SuperTrend DOWN at {st_line.iloc[-1]:.4f}",
        }
        score += 0
    else:
    ...
    if adx_val >= 25:
        ...
        indicators["adx"] = {
            "vote": "WEAK_TREND",
            "strength": 50,
            "detail": f"ADX {adx_val:.1f} (developing)",
        }
        score += 7
    else:
    ...
    if macd_hist > 0 and macd_hist > macd_hist_prev:
        indicators["macd"] = {
            "vote": "BULLISH",
            "strength": 85,
            "detail": f"MACD histogram rising ({macd_hist:.6f})",
        }
        score += 15
        ...
        indicators["macd"] = {
            "vote": "BEARISH",
            "strength": 85,
            "detail": f"MACD histogram falling ({macd_hist:.6f})",
        }
        score += 0
    elif macd_hist > 0:
        indicators["macd"] = {
            "vote": "BULLISH_WEAK",
            "strength": 55,
            "detail": "MACD histogram positive but flat",
        }
        score += 10
    elif macd_hist < 0:
        indicators["macd"] = {
            "vote": "BEARISH_WEAK",
            "strength": 55,
            "detail": "MACD histogram negative but flat",
        }
        score += 5
    else:
    ...
    if vol_ratio >= 2.0:
        ...
        indicators["volume"] = {
            "vote": vol_dir,
            "strength": 80,
            "detail": f"Volume ratio {vol_ratio:.2f}x (high)",
        }
        ...
        indicators["volume"] = {
            "vote": "CONFIRMING",
            "strength": 55,
            "detail": f"Volume ratio {vol_ratio:.2f}x (normal)",
        }
        score += 5
    else:
        indicators["volume"] = {
            "vote": "WEAK",
            "strength": 25,
            "detail": f"Volume ratio {vol_ratio:.2f}x (low)",
        }
        score += 2
    ...
    if 50 <= rsi_val <= 70:
        indicators["rsi"] = {
            "vote": "BULLISH",
            "strength": 70,
            "detail": f"RSI {rsi_val:.1f} (healthy momentum)",
        }
        score += 10
    elif 30 <= rsi_val < 50:
        indicators["rsi"] = {
            "vote": "BEARISH",
            "strength": 70,
            "detail": f"RSI {rsi_val:.1f} (weak momentum)",
        }
        score += 0
    elif rsi_val > 70:
        indicators["rsi"] = {
