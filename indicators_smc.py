...
def detect_order_blocks(df, lookback=20):
    ...
    if n < lookback + 5:
        return blocks
    h, low, c, o = df["h"].values, df["l"].values, df["c"].values, df["o"].values
    t = df["t"].values
    atr_val = _atr(df, 14)
    if atr_val <= 0:
        return blocks
    ...
    for i in range(lookback, n - 3):
        ...
        if c[i] < o[i]:  # bearish candle
            ...
            if move > 1.5 * atr_val:
                ...
                significance = _count_reactions(df, low[i], atr_val, i)
...
def detect_fvg(df, lookback=30):
    ...
    for i in range(max(4, lookback), n - 1):
        ...
        if low[i + 1] > h[i - 1] and atr_val > 0:
            gap_size = low[i + 1] - h[i - 1]
                ...
                fvgs.append(
                    {
                        ...
                        "strength": strength,
                        "timestamp": str(t[i]),
                        "mid": float((low[i + 1] + h[i - 1]) / 2),
                    }
                )

        # Bearish FVG: gap between candle[0] low and candle[2] high
        if h[i + 1] < low[i - 1] and atr_val > 0:
            gap_size = low[i - 1] - h[i + 1]
            if gap_size > 0.3 * atr_val:
                strength = min(100, int(40 + (gap_size / atr_val) * 20))
                fvgs.append(
...
def detect_choch(df, k=3):
    ...
    if len(highs) >= 3 and len(lows) >= 3:
        ...
        h_prices = [h_val[1] for h_val in highs[-4:]]
        low_prices = [low_val[1] for low_val in lows[-4:]]

        # Downtrend: series of lower highs, then break above last LH
        if len(h_prices) >= 3 and h_prices[-2] < h_prices[-3] and c[-1] > h_prices[-1]:
        ...
        if len(low_prices) >= 3 and low_prices[-2] > low_prices[-3] and c[-1] < low_prices[-1]:
            strength = min(100, int(55 + ((low_prices[-1] - c[-1]) / low_prices[-1]) * 400))
            signals.append(
                {
                    "type": "BEARISH_CHOCH",
                    "direction": "SHORT",
                    "level": float(low_prices[-1]),
                    "strength": strength,
                    "timestamp": str(t[-1]),
                }
            )
...
def detect_liquidity_sweep(df, lookback=20):
    ...
    if n < lookback + 3:
        return sweeps
    h, low, c = df["h"].values, df["l"].values, df["c"].values
    t = df["t"].values
    atr_val = _atr(df, 14)

    recent_lows = low[-lookback:-2].min()
    recent_highs = h[-lookback:-2].max()

    # Bullish sweep: wick below recent lows, close above
    if low[-1] < recent_lows and c[-1] > recent_lows and atr_val > 0:
        sweep_depth = (recent_lows - low[-1]) / atr_val
        if sweep_depth > 0.2:
...
def _count_reactions(df, level, atr_val, before_idx):
    ...
    for i in range(start, before_idx):
        if abs(h[i] - level) < 0.5 * atr_val or abs(low[i] - level) < 0.5 * atr_val:
            count += 1
    return count
